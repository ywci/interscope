...
#
# Unit tests for lowering passes: isir_to_assert, assert_to_sva,
# assert_to_vhdl, assert_to_verilog_ovl.

import pytest
from isir.dialects import isir
from isir.dialects import asserts
from isir.lowering import isir_to_assert
from isir.lowering import assert_to_sva
from isir.lowering import assert_to_vhdl
from isir.lowering import assert_to_verilog_ovl
...
def test_isir_to_assert_basic():
    spec_mod = create_test_spec_module()
    assert_mod = isir_to_assert.convert(spec_mod)
...
def test_isir_to_assert_eventually_property():
...
def test_assert_to_sva_with_eventually():
    ...
    spec_mod = isir.SpecModule(name="test", clocks=[], resets=[])
    prop_op = isir.SpecPropertyOp(
        prop_name="eventual_grant",
        kind="liveness",
        expression={"kind": "eventually", "operand": "(read grant)", "bound": 10},
...
def test_assert_to_ovl_unsupported_sequence():
    assert_mod = asserts.AssertModule(name="test")
    seq_op = asserts.AssertSequenceOp(sequence=["req", "##1 grant"])
