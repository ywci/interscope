# ISIR: Specification Intermediate Representation for Hardware Verification
...
ISIR is a **structured intermediate representation** bridging natural‑language hardware design specifications and multiple verification engines (simulation, model checking, interactive theorem proving). It provides:
...
- A **`trace` dialect** to capture simulation results (e.g., from Verilator) and **lift** them back to abstract ISIR events.
...
1. **Semantic precision** – each ISIR construct has a well‑defined meaning (operational or denotational).
...
ISIR uses **YAML 1.2** with a strictly defined subset (no implicit booleans, no flow style except for short sequences). A JSON schema is provided for validation.
...
```yaml
- name: <instance_name>
  module: <module_name>                     # reference to another ISIR module
...
When `backend: kōika`, the lowering pass (`isir → kōika`) generates a Coq theorem and a proof skeleton. When `backend: acl2`, it generates an ACL2 `defthm` and supporting `defun-sk` or `defchoose` forms.
...
All these fields are optional; when absent, the design is treated as uncategorised. The schema (`conf/schemas/isir_schema.yaml`) has been updated to accept them.

---

## 4. Dialect‑Based Layering

ISIR lowering uses **six internal dialects**, each modelled on the MLIR concept of operations and lowering passes. The `proof` dialect has been **removed** – all proof obligations are lowered directly to the Kōika or ACL2 backends.
...
The `isir` dialect module container additionally holds `types`, `components`, and `fairness` lists that mirror the top‑level YAML structure.

### 4.1 Lowering and Lifting Passes (Definitive)

**Lowering (design → implementation)**:
1. **`isir → kōika`** : Translate state, rules, schedule, and directives into Kōika definitions and Coq lemmas/theorems.
2. **`isir → acl2`** : Generate ACL2 functional model (if needed) and proof obligations as `defthm` forms.
3. **`isir → assert`** : Convert properties and directives into the unified `assert` dialect (language‑agnostic).
...
7. **`kōika → rtl`** : Kōika’s verified compiler generates RTL. This pass also emits **source mapping** (RTL signal → ISIR element) as a separate annotation file.
...
9. **`trace → spec`** : Using source mapping, reconstruct abstract state updates, rule firings, and property evaluations from the low‑level `trace` dialect. Output a ISIR‑compatible trace (YAML) that can be checked against original properties.
...
**Lifting Correctness**: The `trace → spec` pass is correct if the lifted abstract trace, when simulated under the original ISIR semantics, produces the same sequence of abstract state updates as the concrete RTL simulation, modulo the mapping provided.

---

## 6. Evidence Binding and Traceability

Every ISIR element may contain an `evidence` field with a list of references:
...
ISIR includes explicit constructs to support LLM‑based translation and iterative repair.
...
## 8. Mapping from ISIR to Kōika Dialect

The following table defines the **canonical lowering** from ISIR constructs to Kōika (Coq‑embedded DSL). This mapping is used in the `spec → kōika` lowering pass. Proof obligations are lowered to Coq lemmas/theorems within the same dialect.

| ISIR Construct | Kōika Target | Notes |
|----------------|--------------|-------|
...
## 9. Mapping from ISIR to ACL2 Dialect

The `acl2` dialect represents ACL2 functional models and theorems. Lowering from ISIR to ACL2 is used for backends that prefer first‑order, rewrite‑based reasoning.

| ISIR Construct | ACL2 Target | Notes |
|----------------|-------------|-------|
...
The `trace` dialect captures cycle‑by‑cycle signal values from simulation (e.g., Verilator VCD) and supports **lifting** back to abstract ISIR events.
...
| `trace.annotation` | Mapping from an RTL signal to a ISIR element | `trace.annotation @full spec_ref = "module.state[name=full]"` |
...
- The original ISIR state, rule, or internal signal name.
- The kind of ISIR element (register, rule condition, rule action, etc.).
...
```json
{
  ...
  "isir_ref": "module.rules[name=do_enqueue].condition",
  "kind": "rule_condition"
}
```

This mapping is embedded in the Verilog as `//@isir` comments or stored separately.
...
2. For each cycle:
   - For each registered `state` signal, translate its bit value to the ISIR type (e.g., `bits<3>` → integer).
...
3. Produce a **ISIR trace** in YAML format:
...
```bash
# 1. Generate RTL from ISIR via Kōika
isir compile fifo.isir --backend kōika --out rtl/fifo.v --mapping fifo.mapping.json
...
isir import vcd --input fifo.vcd --mapping fifo.mapping.json --output fifo.trace.mlir

# 5. Lift to ISIR abstract trace
isir lift --trace fifo.trace.mlir --mapping fifo.mapping.json --output fifo_trace.yaml

# 6. Check properties against the lifted trace
isir check --trace fifo_trace.yaml --spec fifo.isir
```
...
- A description of the ISIR design (states, rules).
- A snippet of the `trace` dialect (several cycles of relevant signals).
- The expected output format (ISIR trace).
...
- Previous discussions on ISIR design, lowering, lifting, and LLM integration.
