# InterScope: Leveraging ISIR for Multi‑Engine Hardware Verification

InterScope is the reference implementation of **ISIR** (InterScope IR), a semi‑formal specification intermediate representation for multi‑engine hardware verification. It bridges natural‑language design specifications with simulation, model checking, and theorem proving (Kōika/Coq and ACL2).

The project provides:

- A **YAML‑based specification language** (`.isir` files) – supports version `0.1`.
...
- **Trace lifting** from Verilator VCD simulations back to abstract ISIR traces.
...
- **PERF (Proof tree Exploration with Reflective Feedback)** – an advanced test‑time proof search that uses tree exploration, Pareto pruning, and LLM reflection to tackle hard proof obligations, with explicit integration of model‑checking counterexamples. PERF is the utilization of ISIR for proof suspension and advanced proof search.
- **Batch processing** of multiple `.isir` files with structured reporting for integration into evaluation pipelines.
...
| Document | Contents |
|----------|----------|
| [`docs/ISIR.md`](docs/ISIR.md) | ISIR language reference: syntax, semantics, dialects, and versioning. |
| [`docs/ARCH.md`](docs/ARCH.md) | Architecture overview: parser, lowering passes, verification engines, and PERF. |
| [`conf/schemas/isir_schema.yaml`](conf/schemas/isir_schema.yaml) | JSON Schema for `.isir` input files. |
| [`conf/schemas/trace_schema.yaml`](conf/schemas/trace_schema.yaml) | JSON Schema for abstract trace YAML produced by `isir lift`. |
...
The `.isir` file format is validated against [`conf/schemas/isir_schema.yaml`](conf/schemas/isir_schema.yaml); abstract trace YAML is validated against [`conf/schemas/trace_schema.yaml`](conf/schemas/trace_schema.yaml).

---

## Usage

The main entry point is the `run.sh` wrapper. All commands are forwarded to the `isir` CLI running inside the uv environment.

### Global Options

| Option | Description |
|--------|-------------|
| `--batch [DIR]` | Process all `.isir` files found recursively in DIR (default: current directory). |
...
| Command | Description |
|---------|-------------|
| `./run.sh --compile <file.isir>` | Compile an ISIR design to Kōika/ACL2/assert dialects and optionally generate RTL. |
| `./run.sh --verify <file.isir>` | Run proof obligations (theorem proving **or** model checking) on the design. |
| `./run.sh --sim <file.isir>` | Compile to RTL and run Verilator simulation (produces a VCD trace). |
| `./run.sh --lift <vcd_file>` | Lift a VCD trace to an abstract ISIR trace (YAML). |
| `./run.sh --check <trace.yaml>` | Check properties against an abstract trace (schema‑validated by default). |
...
```bash
./run.sh compile examples/fifo/fifo.isir
./run.sh verify examples/fifo/fifo.isir --perf
```
...
- `--no-validate` (check) : Skip schema validation of the abstract trace file.
...
**PERF** is a **test‑time proof search** engine that extends the linear repair loop with a **tree‑based beam search**. PERF is the utilization of ISIR for proof suspension and advanced proof search. When a proof attempt fails, PERF:
...
You can override any of these via **environment variables** (see below) or **per‑obligation metadata** in the `.isir` file.
...
```bash
PERF_BEAM_SIZE=5 PERF_DEPTH=3 ./run.sh --verify examples/fifo/fifo.isir --perf
```
...
```bash
./run.sh --verify examples/fifo/fifo.isir --perf --perf-stats
```
...
```bash
./run.sh --compile examples/fifo/fifo.isir --output-format json
./run.sh --verify examples/fifo/fifo.isir --backend koika --output-format json
./run.sh --sim examples/fifo/fifo.isir --cycles 100 --output-format json
```

The JSON output for each command follows a standardised schema defined in `src/isir/utils/result_types.py`. Aggregated batch reports include summary statistics and can be exported to CSV as well.

---

## Evidence Registry Enhancements

The evidence registry (`isir query`) now supports additional sub‑commands for exporting and analyzing verification results:
...
./run.sh --compile examples/fifo/fifo.isir --synthesize
./run.sh --sim examples/fifo/fifo.isir --synthesize
```
...
```bash
cd examples/fifo
../../run.sh --compile fifo.isir
../../run.sh --verify fifo.isir --backend koika
../../run.sh --verify fifo.isir --backend acl2
../../run.sh --sim fifo.isir --cycles 100
../../run.sh --lift build/traces/fifo.vcd --spec fifo.isir
../../run.sh --check build/traces/lifted.yaml --spec fifo.isir
```
...
```bash
./run.sh --verify examples/fifo/fifo.isir --backend model_checking
```

### Assertion Generation (standalone)

```bash
./run.sh --compile examples/fifo/fifo.isir --backend assert --assert-lang sva
./run.sh --compile examples/fifo/fifo.isir --backend assert --assert-lang vhdl
./run.sh --compile examples/fifo/fifo.isir --backend assert --assert-lang verilog_ovl
```
...
### Trace Lifting and Checking

```bash
# Lift a VCD trace produced by simulation
./run.sh --lift build/traces/fifo.vcd --spec examples/fifo/fifo.isir

# Check properties against the lifted trace (schema-validated by default)
./run.sh --check build/traces/lifted.yaml --spec examples/fifo/fifo.isir

# Skip schema validation for a hand-edited trace
./run.sh --check my_trace.yaml --spec examples/fifo/fifo.isir --no-validate
```
