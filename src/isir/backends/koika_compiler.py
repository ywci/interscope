# src/isir/backends/koika_compiler.py
#
...
#
# For the full synthesis pipeline (ISIR → Verilog), use
# ``isir.lowering.koika_to_rtl.convert`` instead—it handles
...
def compile_ocaml_to_verilog(
    ...
    Coq‑to‑OCaml extraction or ISIR‑to‑OCaml generation.
    ...
    mapping = rtl.RTLMapping(design_name=design_name, entries=[])
    mapping_path = output_dir / "mapping.json"
    with open(mapping_path, "w") as f:
        import json
        json.dump(mapping.to_json(), f, indent=2)

    rtl_module = rtl.RTLModule(
        name=design_name,
