# src/isir/verification/proof/proof_skill.py
#
...
from isir.backends.llm_client import get_llm_client_from_config
from isir.verification.proof.proof import ProofSkill, ProofResult
...
from isir.verification.proof.tactic_modernizer import modernize_tactics
from isir.verification.proof.koika.proof_gen import adapt_proof
...
class LLMProofSkill(ProofSkill):
    ...
    def _prove_model_check(self, obligation: Dict[str, Any], context: Dict[str, Any]) -> ProofResult:
        ...
        try:
            from isir.evidence.annotator import create_evidence_ref, add_evidence_to_registry
    ...
    def _build_perf_context(self, obligation: Dict[str, Any], context: Dict[str, Any],
        ...
        # Normalize the backend string so the ACL2 branch is reached even if
        # the obligation uses different case or the accented "Kōika" spelling.
        raw_backend = obligation.get("backend", "koika")
        if isinstance(raw_backend, str):
            backend = raw_backend.lower().replace("ō", "o")
        else:
            backend = "koika"
