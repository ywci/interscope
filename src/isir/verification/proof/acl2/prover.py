# src/isir/verification/proof/acl2/prover.py
#
...
from isir.backends.acl2_client import ACL2Client, get_acl2_client_from_config
from isir.backends.llm_client import LLMClient, get_llm_client_from_config
from isir.verification.proof.acl2.proof_gen import (
...
class ACL2Prover:
    ...
    PERF integration:
    - prove_theorem(..., initial_script=...) accepts a PERF-provided script.
    ...
    def prove_theorem(
        ...
        Args:
            ...
            statement: ACL2 formula. If None, the theorem is assumed to already
                       exist in the session and we attempt to re‑verify it by name.
            hints: Optional list of ACL2 hint s-expressions.
            initial_script: Optional PERF-provided proof script (a complete
                       ``(defthm ...)`` form or a bare ACL2 command). When
                       supplied, it is tried first; if it fails, the normal
                       repair loop runs from scratch.

        Returns:
            ProofResult describing the outcome.
        """
        ...
        if initial_script:
            logger.info(
                "Trying PERF-provided initial script for '%s'.", theorem_name
            )
            try:
                result = acl2.send(initial_script)
                if (not acl2._contains_error(result)
                        and ("Q.E.D." in result or "Proof succeeded" in result)):
                    duration = time.time() - start_time
                    logger.info(
                        "PERF initial script succeeded for '%s'.", theorem_name
                    )
                    return ProofResult(
                        success=True,
                        ...
                        duration=duration,
                        backend="acl2",
                        metadata={"automation": "perf_initial_script"},
                    )
                logger.info(
                    "PERF initial script did not prove '%s'; "
                    "continuing with the normal repair loop.",
                    theorem_name,
                )
                try:
                    acl2.undo()
                ...
                logger.warning("PERF initial script raised: %s", e)
                try:
                    acl2.undo()
                except Exception:
                    pass
        ...
        logger.info(
            "Starting main proof attempt loop for '%s' (max %d attempts).",
            theorem_name,
            self.max_repair,
        )
        ...
        while attempt < self.max_repair:
            ...
            if attempt > 0 and error_key in [err[:80] for err in [last_error]]:
                ...
                if same_error_count >= 3:
                    logger.warning(
                        "The same error has occurred %d times; stopping repair.",
                        same_error_count,
                    )
                    break

            # Strategy reflection at the midpoint of the repair budget.
            if attempt + 1 == self.max_repair // 2:
                logger.info(
                    "Triggering strategy reflection after %d failed attempts.",
                    attempt + 1,
                )
                ...
                if new_approach:
                    ...
                    same_error_count = 0
                    logger.info(
                        "Reflection yielded new hints; resetting failure history."
                    )
                    attempt += 1
                    continue
            ...
            if new_hints:
                logger.info(
                    "LLM returned %d new hint(s): %s", len(new_hints), new_hints
                )
        ...
        return ProofResult(
            success=False,
            error_message=(
                f"ACL2 proof failed after {attempt} attempts: {last_error}"
            ),
            duration=duration,
            backend="acl2",
            iterations=attempt,
    ...
    def _prove_existing_by_name(
        self, acl2: ACL2Client, theorem_name: str, start_time: float
    ) -> ProofResult:
        ...
        for name_variant in (theorem_name, f"acl2::{theorem_name}"):
            ...
            try:
                ...
                logger.debug(
                    "Verification attempt with '%s' raised: %s", name_variant, e
                )
                continue
