# src/isir/verification/perf/perf_traversal.py
#
...
from isir.backends.llm_client import LLMClient, LLMClientError
from isir.verification.perf.perf_config import PERFConfig
...
from isir.verification.perf.perf_analyzer import PERFAnalyzer, ObligationAnalysis
from isir.verification.perf.error_history import ErrorHistory
...
from isir.verification.proof.acl2.proof_gen import extract_acl2_proof
from isir.verification.proof.koika.auto_patcher import auto_patch
from isir.verification.proof.domain_tactics import get_koika_tactic_pattern
from isir.verification.proof.structural_validator import validate_structure
from isir.verification.proof.proof_pattern_cache import get_proof_pattern_cache
...
class PERFTraversal:
            ...
            if shortfall > 0:
                if self.backend.startswith("koi"):
                    ...
                    if domain_script and not self._has_critical_structural_issues(domain_script):
                        ...
                        from isir.verification.proof.koika.template_gen import generate_coq_proof_variants_template
                    ...
                    from isir.verification.proof.acl2.template_gen import generate_acl2_proof_variants_template
    ...
    def _build_prompts_for_parent(
        ...
        if self.backend.startswith("koi"):
            ...
            from isir.verification.proof.acl2.proof_gen import build_acl2_proof_prompt
            for i in range(normal_n):
                hint = default_strategy if default_strategy else tags_to_use[i]
                error_with_loc = self._augment_error_with_location(err) if err else err
    ...
    def _extract_acl2_placeholder(self, acl2_file: Path) -> Optional[str]:
        """Extract a complete (defthm ...) block from an ACL2 file.

        ACL2 symbol names normally use hyphens (fsm-state-self) while ISIR
        properties use underscores (fsm_state_self), so we try several name
        variants before falling back to the first defthm in the file.
        """
        try:
        ...
        def _extract_at(start_index: int) -> Optional[str]:
            snippet = content[start_index:]
            extracted = extract_acl2_proof(snippet)
            if extracted and extracted.strip().startswith("(defthm"):
                return extracted
            return None
        ...
        if theorem_name:
            ...
            for candidate in candidates:
                pattern = re.compile(
                    rf"\(defthm\s+{re.escape(candidate)}\b",
                    re.IGNORECASE | re.MULTILINE,
                )
                m = pattern.search(content)
                if m:
                    result = _extract_at(m.start())
                    if result:
                        return result

        # Fallback: first defthm in the file.
        pattern = re.compile(r"\(defthm\s+\w+", re.MULTILINE)
        m = pattern.search(content)
        if not m:
            return None
        return _extract_at(m.start())
    ...
    def _extract_script_from_response(self, response: str, is_repair: bool = False) -> Optional[str]:
        if self.backend.startswith("koi"):
            from isir.verification.proof.koika.proof_gen import extract_proof_script
    ...
    def _evaluate_koika_node(self, node: PERFNode) -> Dict[str, Any]:
        ...
        try:
            ...
            from isir.backends.rocq_client import RocqClient
    ...
    def _evaluate_acl2_node(self, node: PERFNode) -> Dict[str, Any]:
        ...
        from isir.backends.acl2_client import ACL2Client
        acl2 = ACL2Client(
        ...
        try:
            ...
            # Load the ACL2 definitions file into the session.
            # ACL2Client.load_file wraps `(ld "...")` and returns
            # {"success": bool, "output": str, "error": str|None}.
            load_result = acl2.load_file(Path(acl2_file))
            if not load_result.get("success"):
                return {
                    "success": False,
                    "error": (
                        "Failed to load ACL2 file: "
                        f"{load_result.get('error') or load_result.get('output')}"
                    ),
                }

            # Evaluate the candidate proof script.
            ...
            # Verify the theorem by name.
            verify_result = acl2.send(f"(verify {theorem_name})")
            if acl2._contains_error(verify_result):
                return {"success": False, "error": verify_result}
