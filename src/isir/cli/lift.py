# src/isir/cli/lift.py
#
# CLI subcommand `lift` – converts a VCD trace to an abstract ISIR trace YAML.
...
def _setup_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="isir lift",
        description="Lift a VCD simulation trace to an abstract ISIR trace (YAML)."
    )
    parser.add_argument("vcd", type=str, help="Path to the VCD file")
    parser.add_argument("--spec", "-s", type=str, required=True,
                        help="Path to the original .isir file (for state/rule/interface info)")
    parser.add_argument("--mapping", "-m", type=str, default=None,
                        help="Path to mapping.json (RTL signal to ISIR reference)")
...
def lift_trace(args: argparse.Namespace) -> int:
    ...
    try:
        spec_module = convert_ast_to_isir_module(spec_ast.module)
    except Exception as e:
        logger.error(f"AST to SpecModule conversion failed: {e}")
        return 1

    logger.info("Lifting trace to abstract ISIR format...")
    try:
        abstract_trace = trace_to_isir.convert(trace_mod, spec_module)
    except Exception as e:
