# src/isir/cli/main.py
#
# Central CLI entry point for InterScope.
# Supports batch processing of multiple .isir files, structured output,
...
#
# Examples:
#   python -m isir.cli.main compile examples/fifo/fifo.isir
#   python -m isir.cli.main --batch benchmarks/ --compile --output-format json --report-file results.json
...
from isir.utils.batch import find_isir_files
from isir.utils.reporting import (
    write_json_report,
...
def _parse_global_options(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Process all .isir files in the given directory (default: current directory)."
    )
    parser.add_argument(
...
def _run_single(global_args: argparse.Namespace) -> int:
    ...
    tmp_config = None
    if global_args.config:
        from isir.utils.config_loader import load_config as reload_cfg
        config = reload_cfg()
        fd, tmp_path = tempfile.mkstemp(suffix=".yaml", prefix="isir_merged_")
        os.close(fd)
    ...
    try:
        if cmd == "compile":
            from isir.cli.compile import main as mod_main
        elif cmd == "verify":
            from isir.cli.verify import main as mod_main
        elif cmd == "sim":
            from isir.cli.sim import main as mod_main
        elif cmd == "check":
            from isir.cli.check import main as mod_main
        elif cmd == "lift":
            from isir.cli.lift import main as mod_main
        elif cmd == "query":
            from isir.cli.query import main as mod_main
        elif cmd == "validate-config":
            from isir.cli.validate_config import main as mod_main
...
def _run_batch(global_args: argparse.Namespace) -> int:
    ...
    files = find_isir_files(batch_dir)
    if not files:
        logger.warning("No .isir files found in %s", batch_dir)
        return 0

    logger.info("Batch mode: %d .isir file(s) found.", len(files))
...
def _run_command_on_file(cmd: str, spec_file: Path, global_args: argparse.Namespace) -> Any:
    """
    Execute `isir <cmd> <spec_file>` with JSON output and parse the result
    into the appropriate dataclass.
    """
    args = [sys.executable, "-m", "isir.cli.main", cmd, str(spec_file),
            "--output-format", "json"]
