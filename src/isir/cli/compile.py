# src/isir/cli/compile.py
#
# CLI subcommand `compile` – parses a .isir file, lowers to the
# chosen backend, and generates output (RTL, Coq, ACL2, assertions).
# Uses the canonical AST-to-ISIR conversion and the consolidated
...
from isir.parser.validator import validate_isir_file
from isir.lowering.ast_to_isir import convert_ast_to_isir_module
from isir.lowering.split_rules import split_rules
from isir.utils.logger import setup_logging, get_logger
from isir.utils.config_loader import load_config, get_project_root
from isir.utils.result_types import CompilationReport, BackendResult
...
def _setup_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="isir compile",
        description="Compile a ISIR specification to various backends."
    )
    parser.add_argument("input", type=str, help="Path to the .isir file")
...
def compile_spec(args: argparse.Namespace) -> int:
    ...
    try:
        validate_isir_file(input_path)
    except Exception as e:
        logger.error(f"Schema validation failed: {e}")
        return 1

    try:
        ast_doc = parse_isir(input_path)
    except Exception as e:
    ...
    try:
        spec_module = convert_ast_to_isir_module(ast_doc.module)
    except Exception as e:
...
def _run_simulation(spec_module, out_dir, args, config) -> int:
    ...
    try:
        ...
        logger.info(f"Next: isir lift {vcd_file} --mapping {out_dir / 'rtl' / 'mapping.json'}")
        return 0
...
def _compile_koika(spec_module, out_dir, args, config) -> List[BackendResult]:
    ...
    from isir.lowering.isir_to_koika import convert as isir_to_koika_convert
    from isir.lowering.koika_to_rtl import convert as koika_rtl_convert
    ...
    start = time.time()
    try:
        koika_mod = isir_to_koika_convert(spec_module)
        coq_dir = out_dir / "coq"
    ...
    if not args.no_rtl:
        logger.info("Lowering ISIR → RTL (Kōika synthesis)...")
        start = time.time()
        rtl_dir = out_dir / "rtl"
...
def _compile_acl2(spec_module, out_dir, args) -> List[BackendResult]:
    from isir.lowering.isir_to_acl2 import convert as isir_to_acl2_convert

    logger.info("Lowering spec → acl2...")
    start = time.time()
    try:
        acl2_mod = isir_to_acl2_convert(spec_module)
...
def _compile_assert(spec_module, out_dir, args) -> List[BackendResult]:
    ...
    from isir.lowering.isir_to_assert import convert as isir_to_assert_convert
    from isir.lowering.assert_to_sva import convert as assert_to_sva_convert
    from isir.lowering.assert_to_vhdl import convert as assert_to_vhdl_convert
    from isir.lowering.assert_to_verilog_ovl import convert as assert_to_verilog_ovl_convert

    logger.info("Lowering spec → assert...")
    start = time.time()
    try:
        assert_mod = isir_to_assert_convert(spec_module)
    except Exception as e:
...
def _generate_coq_from_module(koika_mod) -> str:
    ...
    lines = [f"(* Generated from ISIR design {koika_mod.name} *)", "Require Import Koika.", ""]
...
def _generate_acl2_from_module(acl2_mod) -> str:
    ...
    parts = [f";; Generated from ISIR design {acl2_mod.name}"]
