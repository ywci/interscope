# src/isir/lowering/assert_to_verilog_ovl.py
#
...
def _ovl_expr(expr_str: str) -> str:
    """
    Convert a ISIR S‑expression string into a Verilog boolean expression.
...
def _resolve_clock(op_clock: Optional[str],
                   module_clock: Optional[asserts.AssertClockOp]) -> str:
...
def _resolve_reset(op_reset: Optional[str],
                   module_reset: Optional[asserts.AssertResetOp]) -> str:
    """
...
def _always_to_ovl(always_op: asserts.AssertAlwaysOp,
                   instance_counter: int,
                   module_clock: Optional[asserts.AssertClockOp],
                   module_reset: Optional[asserts.AssertResetOp]) -> str:
...
def _cover_to_ovl(cover_op: asserts.AssertCoverOp,
                  instance_counter: int,
                  module_clock: Optional[asserts.AssertClockOp],
                  module_reset: Optional[asserts.AssertResetOp]) -> str:
...
def convert(assert_module: asserts.AssertModule) -> str:
    ...
    lines = []
    lines.append("// Generated by InterScope from ISIR assertion dialect (OVL backend)")
