# src/isir/lowering/isir_to_assert.py
#
...
from isir.dialects import isir, asserts
...
def _build_clock_op(clocks) -> Optional[asserts.AssertClockOp]:
    ...
    return asserts.AssertClockOp(clock_name=name, edge=edge)


def _build_reset_op(resets) -> Optional[asserts.AssertResetOp]:
    ...
    if polarity == "active_low":
        ...
        cond = name
    return asserts.AssertResetOp(reset_condition=cond)


def _convert_temporal_expr(expr: dict) -> asserts.AssertPropertyOp:
    """
    ...
    if kind == "always":
        ...
        if operand and ("implies" in operand or "->" in operand or "|->" in operand):
            return asserts.AssertPropertyOp(kind="always", operand=operand, bound=bound)
        else:
            return asserts.AssertAlwaysOp(condition=operand)

    elif kind == "eventually":
        return asserts.AssertPropertyOp(kind="eventually", operand=operand, bound=bound)
    elif kind == "until":
        return asserts.AssertPropertyOp(kind="until", left=left, right=right, bound=bound)
    else:
        raise ValueError(f"Unknown temporal kind: {kind}")


def convert(spec_module: isir.SpecModule) -> asserts.AssertModule:
    """
    ...
    for prop_op in spec_module.property_ops:          # SpecPropertyOp
        ...
        if isinstance(assert_op, asserts.AssertAlwaysOp):
