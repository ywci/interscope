...
def _parse_vcd(vcd_path: Path) -> trace.TraceModule:
    """
    ...
    """
    trace_mod = trace.TraceModule(
        module_op=trace.TraceModuleOp(trace_name=vcd_path.stem)
    )
    ...
    for name, width in signals_info.items():
        trace_mod.signals.append(trace.TraceSignalOp(signal_name=name, width=width))

    if clock_id:
        clock_name = id_to_name[clock_id]
        trace_mod.clock = trace.TraceClockOp(clock_name=clock_name, edge="posedge")
    else:
...
def convert(vcd_path: Path, mapping_file: Optional[Path] = None) -> trace.TraceModule:
    """
    Convert a VCD file to a TraceModule.

    Args:
        vcd_path: Path to the VCD file.
        mapping_file: Optional JSON file with RTL‑to‑ISIR mappings.
