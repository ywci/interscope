# src/isir/lifting/trace_to_isir.py
#
# Lifts a TraceModule (from VCD) to an abstract ISIR trace YAML.
# Uses annotations to map RTL signals to ISIR state variables,
...
# Regex to extract the name from a isir_ref like "module.rules[name=do_enqueue].condition"
_REF_NAME_PATTERN = re.compile(r"\[name=([^\]]+)\]")


def _extract_name_from_ref(isir_ref: str) -> Optional[str]:
    """Extract the element name from a isir_ref string, e.g. ``module.state[name=head]``."""
    match = _REF_NAME_PATTERN.search(isir_ref)
...
def _get_signal_value(cycle_data: trace.TraceCycleData, signal_name: str) -> Any:
    """Return the value of a signal in a cycle, or None if missing."""
    return cycle_data.values.get(signal_name)


def _iface_name_and_type(iface: Union[isir.Interface, Dict[str, Any]]) -> Tuple[str, str]:
    """
    Return the (name, data_type) tuple for an interface description.

    Accepts either a `isir.Interface` dataclass or a plain dictionary
...
def _reconstruct_state(
    state_ops: List[isir.SpecStateOp],
    annotation_map: Dict[str, str],
    cycle_data: trace.TraceCycleData,
...
def _reconstruct_io(
    interfaces: List[Union[isir.Interface, Dict[str, Any]]],
...
def convert(
    trace_module: trace.TraceModule,
    spec_module: isir.SpecModule,
    strict: bool = False
) -> Dict[str, Any]:
    """
    Convert a TraceModule (with annotations) to an abstract ISIR trace.
    ...
    annotation_map: Dict[str, str] = {}          # isir_ref -> signal_name
    rule_cond_signals: Dict[str, str] = {}       # rule_name -> signal_name
    io_signals: Dict[str, str] = {}              # isir_ref -> signal_name

    for ann in trace_module.annotations:
        annotation_map[ann.isir_ref] = ann.signal_name
        if ann.kind == "rule_condition":
            rule_name = _extract_name_from_ref(ann.isir_ref)
            if rule_name:
                rule_cond_signals[rule_name] = ann.signal_name
        elif ann.kind in ("input", "output"):
            io_signals[ann.isir_ref] = ann.signal_name
