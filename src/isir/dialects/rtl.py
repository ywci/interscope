...
# to ISIR elements (used for trace lifting).
...
class MappingEntry:
    """
    Maps an RTL signal to a ISIR element.
    ...
    PERF-specific fields enable fine-grained trace alignment:
      ...
      - expression: The original ISIR expression (for combinational signals).
...
class RTLMapping:
    ...
    def filter_for_obligation(self, obligation: Union[Dict[str, Any], str]) -> List[MappingEntry]:
        ...
        for entry in self.entries:
            ...
            if prop_name in entry.isir_ref:
                filtered.append(entry)
                continue
    ...
    def build_property_signal_index(self) -> Dict[str, List[str]]:
        ...
        for entry in self.entries:
            ...
            import re
            match = re.search(r"property\[name=([^\]]+)\]", entry.isir_ref)
            if match:
