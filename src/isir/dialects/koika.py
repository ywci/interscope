...
from isir.dialects.isir import Dialect, Operation, Type
...
class KoikaModule:
    ...
    information.  They are derived from the ISIR specification and can be
    ...
    inputs: List[Any] = field(default_factory=list)    # Interface objects (isir.Interface or dicts)
...
def from_spec_module(spec_module) -> KoikaModule:
    ...
    ``isir.lowering.isir_to_koika.convert``.
    """
    from isir.lowering.isir_to_koika import convert as isir_to_koika_convert
    return isir_to_koika_convert(spec_module)
