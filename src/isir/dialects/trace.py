# src/isir/dialects/trace.py
#
# Trace dialect – captures cycle-by-cycle simulation data (VCD, etc.)
# and supports lifting back to abstract ISIR events.
...
class TraceAnnotationOp(Operation):
    """
    Mapping from an RTL signal to a ISIR element (for lifting).
    ...
    def __str__(self) -> str:
        base = f"trace.annotation @{self.signal_name} -> {self.isir_ref} ({self.kind})"
