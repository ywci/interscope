...
def _expr_to_coq(expr: Any, state_types: Dict[str, str],
    ...
    if isinstance(expr, bool):
        # In an action context (as_prop=False) we want "true"/"false".
        # If as_prop=True, this branch is still correct because the only
        # place where as_prop=True is used is for rule conditions, where
        # Python bools are generated by the parser from "true"/"false"
        # condition strings.  However, those conditions are always
        # rendered as Coq True/False, which is what we want for Prop.
        if as_prop:
    ...
    # slice
        ...
        if len(args) != 3: raise ExprError("slice expects 3 arguments: expr high low")
    ...
    # read
        ...
        name = args[0]
        if not isinstance(name, str): raise ExprError("read expects a state name string")
        if name in state_types:
    ...
    # mem_read
        ...
        if len(args) != 2: raise ExprError("mem_read expects 2 arguments")
        mem_name = args[0]
        if not isinstance(mem_name, str): raise ExprError("mem_read expects memory name string")
    ...
    for action in rule.actions:
        ...
        if isinstance(parsed, list) and len(parsed) >= 3 and parsed[0] == 'write':
            ...
            if isinstance(field, str) and field in state_names:
                try:
                    # Action values are always in bool/nat context, never Prop
