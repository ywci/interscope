...
def _build_sequential_step(ordered_rules, registers, memories, state_types, input_types, memory_names, state_names):
    ...
    for idx, rule in enumerate(ordered_rules):
        ...
        rule_ops.append(koika_ir.KoikaRuleOp(rule_name=rule.rule_name, condition=cond_str, actions=rule.actions))
...
def convert(spec_module: spec_ir.SpecModule) -> koika_ir.KoikaModule:
    ...
    for po in spec_module.proof_obligations:
        ...
        state_defs.append("Admitted.  (* placeholder – use specir verify to attempt proof *)")
        state_defs.append("")
